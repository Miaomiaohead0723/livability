 ...
 *  
 * *********************************************************************/
var visParam = {min:300, max:2000, 
                palette:["green", "blanchedalmond", "orange","black"]};
                
var visParam2 = {min:0, max:20, 
                palette:["green", "blanchedalmond", "orange","black"]};
...
var visParamMSDI = {min:0.85, max:1.1, 
                palette:["green", "blanchedalmond", "orange","black"]};
...
var visGDP = require('users/samapriya/utils:palettes');
...
var ERA5 = ee.ImageCollection("ECMWF/ERA5_LAND/MONTHLY_AGGR").filter(filter).median().clip(roi);
var pop = ee.ImageCollection('WorldPop/GP/100m/pop').filter(ee.Filter.date('2015-01-01', '2015-12-31')).median().clip(roi);
...
var normalSoil=normalization(soil,roi,scale_my)
...
var normalErosion = normalization(erosion,roi,scale_my)
var normalRDLS = normalization(RDLS,roi,scale_my)
var normalWCI = normalization(WCI,roi,scale_my)
var normalICHB = normalization(ICHB,roi,scale_my).subtract(1).multiply(-1)
var normalmsdi = normalization(msdi,roi,scale_my).subtract(1).multiply(-1)
var normalndvi = normalization(ndvi,roi,scale_my)
var normalpop = normalization(pop,roi,scale_my)
var normalGDP = normalization(GDP,roi,scale_my)
...
Map.addLayer(normalErosion, visRate, 'normalErosion');
Map.addLayer(normalRDLS, visRate, 'normalRDLS');
Map.addLayer(normalWCI, visRate,'normalWCI')
Map.addLayer(normalICHB, visRate,'normalICHB')
Map.addLayer(normalmsdi, visRate, 'normalmsdi');
Map.addLayer(normalndvi,visRate,'normalndvi')
Map.addLayer(normalpop, visRate, 'normalpop');
Map.addLayer(normalGDP,visRate,'normalGDP')
Map.addLayer(eeq, visRate, 'livability');

 Export.image.toDrive({
  image:normalErosion.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalErosion',
  description:year+'normalErosion',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
...
Export.image.toDrive({
  image:normalRDLS.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalRDLS',
  description:year+'normalRDLS',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
...
Export.image.toDrive({
  image:normalWCI.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalWCI',
  description:year+'normalWCI',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
...
Export.image.toDrive({
  image:normalICHB.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalICHB',
  description:year+'normalICHB',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
...
Export.image.toDrive({
  image:normalmsdi.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalmsdi',
  description:year+'normalmsdi',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
...
Export.image.toDrive({
  image:normalpop.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalpop',
  description:year+'normalpop',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
...
Export.image.toDrive({
  image:normalndvi.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalndvi',
  description:year+'normalndvi',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
...
Export.image.toDrive({
  image:normalGDP.clip(roi),
  folder:'EEQ',
  fileNamePrefix :year+'normalGDP',
  description:year+'normalGDP',
  region:roi,
  scale:30,
  crs:"EPSG:4326",
  maxPixels:1e13
